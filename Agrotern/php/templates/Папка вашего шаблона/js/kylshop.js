$(function(){
    ...
    function AddGoods(id, title, price, link, amount){
        ...
        if(Goods !== null){
            ...
            if(Goods[id] !== undefined){ // если товар с таким id уже есть в корзине
                ...
                //#####################################################
                $(".listing-cart tr:last").after('<tr class="oneGoods"><td><a href="'+link+'" data-goodsId="'+id+'">'+title+'</a></td><td><input type="number" name="" class="goodsAmount" min="1" value="'+amount+'"></td><td class="productPrice"><b>'+price+'</b><span> руб.</span><span class="removeGoods"></span></td></tr>');
            }
    ...
    for (var i in GetGoods()){
        $(".listing-cart tr:last, .staticCart tr:last").after('<tr class="oneGoods"><td><a href="'+GetGoods()[i]["link"]+'" data-goodsId="'+[i]+'">'+GetGoods()[i]["title"]+'</a></td><td><input type="number" name="" class="goodsAmount" min="1" value="'+GetGoods()[i]["amount"]+'"></td><td class="productPrice"><b>'+GetGoods()[i]["price"]+'</b><span> руб.</span><span class="removeGoods"></span></td></tr>');
    }
